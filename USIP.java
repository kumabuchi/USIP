...
public class USIP {
	...
	public static final int SAMPLING_NUM = 20000;
        ...
        try {
            ...
            while( (line = br.readLine()) != null ){
                    ...
                    for(int j=0; j<datas.size(); j++){
                            if( datas.get(j).getId().equals(name) ){
                                    positives.add(new Double(datas.remove(j).getValue()));
                                    break;
                            }
                    }
            }
	...
	private void execSampling(){
		...
		for(int i=0; i<SAMPLING_NUM; i++){
			...
			System.out.print("\r# sampling "+(i+1)+" of "+SAMPLING_NUM+" datas ...");
	...
	private void reCalcScore(double smpVal){
		for(int i=0; i<datas.size(); i++){
			datas.get(i).addSumSmpDist(Math.abs(smpVal-datas.get(i).getValue()));
		}
	}
