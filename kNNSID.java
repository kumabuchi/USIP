 ...
 * 2クラスラベル付きの不均衡なデータセットから、その分布を考慮して未ラベル事例をサンプリングするプログラム
...
public class kNNSID {
     ...
     * 各配列の初期化を行います。
     */
    public kNNSID(String dataFile, String posFile){
    ...
    private void reCalcScore(double smpVal){
        for(int i=0; i<datas.size(); i++){
            datas.get(i).setAvgSmpDist(Math.abs(smpVal-datas.get(i).getValue())); // k-NN平均距離
    ...
    public static void main(String[] args){
        ...
        if( args.length != 2 ){
            System.out.println("[USAGE] java kNNSID <UNLABELED_DATA_FILE> <MINORITY_CLASS_DATA_FILE>");
            System.exit(-1);
        }

        kNNSID ins = new kNNSID(args[0], args[1]);
        ins.run();
    ...
    class Data {
        ...
        // 距離計算に用いるサンプリング済事例との距離を保持する配列
        private ArrayList<Double> smpDists;
        ...
        public Data(String id){
            ...
            this.avgPosDist = 0;
            this.score = 0;
            this.smpDists = new ArrayList<Double>();
        }
        ...
        public void setAvgSmpDist(double smpDist){
            if( this.smpDists.size() < 3 ){
                smpDists.add(smpDist);
                setSumSmpDist((this.sumSmpDist*(smpDists.size()-1)+smpDist)/smpDists.size());
                return ;
            }
            int index = -1;
            double maxVal = -1;
            for( int i=0; i<this.smpDists.size(); i++ ){
                if( smpDists.get(i) > maxVal ){
                    maxVal = smpDists.get(i);
                    index = i;
                }
            }
            if( smpDist < smpDists.get(index) ){
                smpDists.remove(index);
                smpDists.add(smpDist);
                setSumSmpDist((this.sumSmpDist*(smpDists.size()-1)+smpDist)/smpDists.size());
            }
        }
        ...
        private void calcScore(){
            ...
            this.score = - this.avgPosDist + this.sumSmpDist;
        }
